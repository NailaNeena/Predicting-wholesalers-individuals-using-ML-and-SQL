...
SELECT 
    ...
    `r`.`race`,
    `ct`.`country_code` AS 'original_country',
...
INNER JOIN
	(SELECT DISTINCT
		...
		SUM(`il`.`quantity`) AS `total_qty_products`, 				-- Getting the sum of all products
        MAX(`il`.`quantity`) AS `max_total_qty_per_product`, 		-- Getting the maximum quantity purchased for a certain product
		AVG(`il`.`quantity`) AS `avg_total_qty_per_product`, 		-- Getting the average quantity purchased for a certain product
        SUM(`il`.`quantity` * `il`.`unit_price`) AS `total_price`,	-- Getting the total price paid by a customer
		`i`.`customer_id`
	FROM
...
ORDER BY `total_qty_products` DESC;
;